...
calendarEventsRouter.get("/:id", async (req, res) => {
    try {
        ...
        const [event] = await (0, db_1.query)(`SELECT * FROM calendar_events WHERE id = ?`, [
            id,
        ]);
        if (!event) {
            res.status(404).json({
...
calendarEventsRouter.post("/", async (req, res) => {
    try {
        ...
        data.forEach((item, index) => {
            ...
            valuesArray.push(item.summary, item.start, item.end, item.description, item.location, item.uid, item.rawString);
        });
        const dbResponse = await (0, db_1.execute)(`
            ...
            ${valuesString}
            `, valuesArray);
        const newRows = await (0, db_1.query)(`SELECT * FROM calendar_events WHERE uid in (${data
            .map(() => "?")
            .join(",")})`, data.map((calendarEvent) => calendarEvent?.uid));
        res.status(201).json({
...
calendarEventsRouter.put("/:id", async (req, res) => {
    try {
        ...
        const dbResponse = await (0, db_1.execute)(`UPDATE calendar_events
        ...
        const [updatedRow] = await (0, db_1.query)(`SELECT * FROM calendar_events WHERE id = ?`, [calendarEventId]);
        res.status(200).json({
...
calendarEventsRouter.delete("/:id", async (req, res) => {
    try {
        ...
        const dbResponse = await (0, db_1.execute)(`DELETE FROM calendar_events WHERE id = ?`, [calendarEventId]);
        res.status(200).json({
