...
schedulerRoutes.get("/", async (req, res) => {
    try {
        const scheduledActions = await scheduler_service_1.SchedulerService.getAllScheduledActions();
        res.status(200).json({
...
schedulerRoutes.get("/:id", async (req, res) => {
    try {
        const scheduledAction = await scheduler_service_1.SchedulerService.getScheduledActionById(parseInt(req.params.id ?? ""));
        res.status(200).json({
...
schedulerRoutes.post("/", async (req, res) => {
    try {
        const newRow = await scheduler_service_1.SchedulerService.createScheduledAction(req.body);
        res.status(201).json({
...
schedulerRoutes.put("/:id", async (req, res) => {
    try {
        const updatedRow = await scheduler_service_1.SchedulerService.updateScheduledAction(parseInt(req.params.id ?? ""), req.body);
        res.status(200).json({
...
schedulerRoutes.patch("/:id", async (req, res) => {
    try {
        const updatedRow = await scheduler_service_1.SchedulerService.patchScheduledAction(parseInt(req.params.id ?? ""), req.body);
        res.status(200).json({
...
schedulerRoutes.delete("/:id", async (req, res) => {
    try {
        const dbResponse = await scheduler_service_1.SchedulerService.deleteScheduledAction(parseInt(req.params.id ?? ""));
        // Send response
        res.status(200).json({
